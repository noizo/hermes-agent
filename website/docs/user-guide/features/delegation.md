...
Claude Code bridge workers run with a strict per-session MCP config. By default that config contains only the worker bridge MCP so the child can report back to Hermes. If `lean_ctx.expose_to_bridge_workers` is active, Hermes also adds its packaged bridge-safe lean-ctx facade and allows only the configured `lean_ctx.bridge_safe_tools`. For other worker MCPs, add servers explicitly with `delegation.bridge_extra_mcp_servers` and expose only the required Claude tools through `delegation.bridge_extra_allowed_tools`. Do not forward the whole project MCP surface unless every server is intended for child workers.

Cursor Agent bridge workers use the project Cursor MCP configuration (`.cursor/mcp.json`) together with `--approve-mcps`; Hermes writes the worker bridge entry and merges configured `bridge_extra_mcp_servers` into that project config for the session, including the same bridge-safe lean-ctx facade when enabled. Cursor Agent does not use Claude Code's `--mcp-config`, `--strict-mcp-config`, or `--allowedTools` flags; absence of those flags in Cursor process arguments is expected, not proof that MCPs are unavailable.
